...
export default function NewCompanyForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setMessage(uiText.documents.enterCompany)
      return
    }
  ...
  return (
    ...
    >
      <h2 className="text-xl font-black tracking-tight text-[#1f1a14]">
        {uiText.documents.newCompany}
      </h2>
      <p className="mt-2 text-sm text-[#7b746b]">
        {uiText.documents.newCompanySubtitle}
      </p>
      ...
      <div className="mt-4 flex flex-col gap-3 md:flex-row">
        <input
          ...
          placeholder={uiText.documents.companyNamePlaceholder}
        ...
        >
          {isSaving ? uiText.documents.adding : uiText.documents.addCompany}
        </button>
      </div>
